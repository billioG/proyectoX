...
async function showMonthlyActivityReport() {
    ...
    try {
        ...
        // Fetch waivers for this month with all details
        ...
        if (error) {
            ...
            const { data: fallbackWaivers } = await _supabase
                .from('attendance_waivers')
                .select('*')
                .gte('created_at', startOfMonth.toISOString())
                .order('created_at', { ascending: false });

            const { data: teachers } = await _supabase.from('teachers').select('id, full_name, email, phone');
            const { data: schools } = await _supabase.from('schools').select('code, name, address');

            waivers = (fallbackWaivers || []).map(w => ({
                ...w,
                teachers: teachers?.find(t => t.id === w.teacher_id) || { full_name: 'Docente', email: '', phone: '' },
                schools: schools?.find(s => s.code === w.school_code) || { name: w.school_code || 'Sin establecimiento', code: w.school_code, address: '' }
            }));
        }
